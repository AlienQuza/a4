...
def cost_nc(patch1, patch2):
    ...
    wlm=np.mean(patch1)
    wpm=np.mean(patch2)
    rob = np.mean((patch1 - wlm) * (patch2 - wpm))
...
def cost_function(patch1, patch2, alpha):
    ...
    # Your code goes here
    #
    cost_val = ((1/range(patch1) )*cost_ssd(patch1, patch2))+(alpha*cost_nc(patch1, patch2))
...
def pad_image(input_img, window_size, padding_mode='symmetric'):
    ...
    padded_img = np.pad(input_img,window_size,padding_mode)
...
def compute_disparity(padded_img_l, padded_img_r, max_disp, window_size, alpha):
    ...
    # Your code goes here
    #
    robr,robl=np.empty(window_size,window_size)

    disparity = padded_img_l.copy()
    for i in range(padded_img_l.shape[0]):  
        for j in range(padded_img_l.shape[1]):
             for a in window_size:
                 for b in window_size:
                    robl[a][b]=padded_img_l[i+a][j+b]
                    robr[a][b]=padded_img_r[i+a][j+b]
        disparity[i][j]=cost_function(robl,robr,alpha)  
